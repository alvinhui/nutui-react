...
本篇介绍如何使用 `@dongweb/nutui-react-taro-cli` 从命令行查询 NutUI-React（Taro 多端）的组件知识。

## 什么是 NutUI-React Taro CLI？

[@dongweb/nutui-react-taro-cli](https://www.npmjs.com/package/@dongweb/nutui-react-taro-cli) 是面向 AI Coding 的 **NutUI-React Taro 离线知识查询 CLI**。它把组件的 Props、文档、Taro 示例与 Design Token 打包随包分发，让 AI 编程助手（Claude Code / Cursor / Copilot 等）从「猜 API」变成「查 API」，从根源消除 API 幻觉。

服务于 `@nutui/nutui-react-taro`（Taro 跨端 / 小程序场景）。H5 场景请使用 [@dongweb/nutui-react-cli](/#/zh-CN/ai/cli)（H5 站点）。
...
nutui-react-taro migrate 3 4 --apply ./src # 扫描项目并生成 v3 → v4 迁移提示
nutui-react-taro diff 3 4 Empty           # 对比 Empty 在 v3 / v4 的 Props 差异
...
| `nutui-react-taro migrate [from] [to]` | 输出大版本迁移指南（默认 `3 4`）；支持 `--component <Component>` 查看单组件，或 `--apply <dir>` 扫描项目并生成迁移提示 |
| `nutui-react-taro diff <v1> <v2> [Component]` | 对比两个版本快照的 Props 差异，包括新增、移除、类型和默认值变更 |
...
完整升级流程可安装 [nutui-react-taro-v3-to-v4 Skill](/#/zh-CN/ai/skill)，由 Agent 按「升级依赖 → 扫描盘点 → 逐组件改写 → 多端构建与视觉验证」执行。
