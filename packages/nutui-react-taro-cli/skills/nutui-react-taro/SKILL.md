...
6. **版本优先自动检测** —— CLI 内置 v3 / v4 多版本快照。在有 NutUI 依赖的项目里工作时，优先**不传** `--nutui-version`，让 CLI 从项目 `node_modules` / `package.json` 自动推断；只有当用户明确要查某个特定版本、或不在项目上下文中时，才显式传 `--nv <version>`。输出里的 `_meta.version` / `_meta.source` 会告诉你实际命中的版本。
